...
<?php 		
	...
	if(z_text_enthaelt_steuerzeichen($nickname) == true){
	    z_html_warnung("Der Nickname darf keine Dollarzeichen, Semikolons, Anfuehrungszeichen und Backslashes enthalten.");
	    zurueckButtonAusgeben();
	    require_once 'footer.php';
	    die();
	}
	...
	{
	    z_html_warnung("Der Nickname muss mindestens 3 Zeichen haben!");
	    zurueckButtonAusgeben();
	    require_once 'footer.php';
	    die();
	}
	
	if(strtolower($nickname) == "nickname")
	{
	    z_html_warnung("Der Nickname darf nicht \"nickname\" sein.");
	    zurueckButtonAusgeben();
	    require_once 'footer.php';
	    die();
	}
	...
	$statement = $z_db_verbindung->prepare(
	    "UPDATE $z_user_mit_db_prefix SET passwort = ?, login = ?, nickname = ?, token_verbraucht = 1 WHERE token = ?"
	);
	$statement->bind_param("sssi",$passwortSHA1,$login,$nickname, $token);
	$statement->execute();
	$statement->close();
